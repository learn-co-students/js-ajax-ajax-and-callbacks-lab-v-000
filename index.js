...
var renderSearchResult = (result) => {
  return `
    <div>
      <h2><a href="${results.html_url}">${result.name}</a></h2>
      <p><a href="#" data-repository="${result.name}" data-owner="${result}"
  `
}

var renderSearchResults = (data) => data.items.map( result => renderSearchResult(result))

var displayError = () => $('#errors').html("I'm sorry, there is an error.")
