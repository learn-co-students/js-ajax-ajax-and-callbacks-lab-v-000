...
function renderResponse(response, item, html){
 ...
 console.log(html);
 if (item === "commits"){
  let results = response.map(response => `${coverGirl(response,item)}`)
  $(`${html}`).html(results)
 }
 if (item === "repo"){
  let results = response.items.map(response => `${coverGirl(response,item)}`)
  $(`${html}`).html(results)
  }
 }

function coverGirl(response, item){
 if (item === "repo"){
   return `
